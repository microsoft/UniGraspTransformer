...
def train_offline_model_batch(args):
    ...
    OTDataset = ObjectTrajectoryDataset(trajectory_dir=trajectory_dir, target_object_lines=target_object_lines, 
                                        sample_ratio=2/args.batch_size, sample_object=True if args.mode=='Object' else False)
    ...
    for nepoch in range(train_epochs + 1):
        ...
        for niter, batch_traj in enumerate(OTDataLoader):
            ...
            for mbatch in range(group_size):
                ...
                valids = torch.tensor(batch_traj['valids'][:, group_index]).to(device) # [Nbatch, 200, 1]
                rendered_pc = torch.tensor(batch_traj['rendered'][:, group_index, :, :num_point, :3]).to(device) #[Nbatch, 200, 1024, 3]
        ...
        if nepoch % save_rate == 0:
            torch.save(PointNetModel.state_dict(), os.path.join(log_dir, 'model_{}.pt'.format(nepoch)))
